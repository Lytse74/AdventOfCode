    ...
    for (auto const& [key, val] : edge_cost ) {
      ...
      tmp2 = make_pair(make_pair(get<0>(key.first),get<1>(key.first)),make_pair(get<2>(key.first),get<3>(key.first)));
        if ( path_cost[tmp] >= path_cost[tmp2] )
          continue;
        cout << minKey.first.first << "|" << minKey.first.second << "(" << minKey.second.first << "," << minKey.second.second << ") " << "spath" << " C:" << path_cost[tmp2] << " : ";
        cout << tmp.first.first << "|" << tmp.first.second << "(" << tmp.second.first << "," << tmp.second.second << ")" << " C:" << val << endl;
