...
int HandleSignal( tuple<char,vector<string>,map<string,bool>,bool> & node, bool signal,string sender, string orig,vector<tuple<string,bool,string>> & signals, int ibut )
{
...
int PushButton( map<string,tuple<char,vector<string>,map<string,bool>,bool>> & nodes, vector<tuple<string,bool,string>> & signals, int & hpulse, int & lpulse,  int ibut )
{
  int ret = 1;
  ...
  while ( it != signals.end() )
  {
    ...
    int lRet = HandleSignal( nodes[recv],val,send,recv, signals, ibut);
    if ( lRet > 1)
      ret = lRet;
    it = signals.begin();
    it = signals.erase(it);
...
int main() {
  ...
  {
    long long lRet = PushButton( nodes, signals, hpulse, lpulse, i+1 );
    if (i<1000)
