...
void calculate( pair<int,int> start )
{
  // find shortest path
  ...
  while ( todo.size() > 0 )
  {
    ...
    if ( tmp.first > 0 )
    {
      ...
      {
        if ( steps[tmp.first-1][tmp.second] == -1 || steps[tmp.first-1][tmp.second] -1 > steps[tmp.first][tmp.second] ) 
        {
    ...
    if ( tmp.second < 161 && input[tmp.first][tmp.second+1] != -1 )
    {
      ...
      {
        if ( steps[tmp.first][tmp.second+1] == -1 || steps[tmp.first][tmp.second+1] -1 > steps[tmp.first][tmp.second] ) 
        {
...
int main() {
  ...
  { 
    for ( auto j=0; j<162; ++j )
    {
      if ( input[i][j] == 'a' )
      {
        calculate( make_pair( i,j ) );
        if ( 0==0
          && steps[end.first][end.second] != -1 
          && steps[end.first][end.second] <= mina )
        {
          mina = steps[end.first][end.second];
          cout << '*';
          cout << "(" << i << "," << j << ")=" << mina;
        }
        initSteps();
